class DeepCacheSDHelper(object):
    ...
    def _is_skip_step(self, block_i, layer_i, blocktype ="down"):
    ...
    def _wrap_modules(self):
        ...
        for block_i, block in enumerate(self.pipe.unet.down_blocks):
            for (layer_i, attention) in enumerate(getattr(block, "attentions", [])):
                self._wrap_block_forward(attention, "attentions", block_i, layer_i)
            for (layer_i, resnet) in enumerate(getattr(block, "resnets", [])):
                self._wrap_block_forward(resnet, "resnet", block_i, layer_i)
            for downsampler in getattr(block, "downsamplers", []) if block.downsamplers else []:
                self._wrap_block_forward(downsampler, "downsampler", block_i, len(getattr(block, "resnets", [])))
            self._wrap_block_forward(block, "block", block_i, 0, blocktype ="down")
        # 3. wrap midblock forward
        self._wrap_block_forward(self.pipe.unet.mid_block, "mid_block", 0, 0, blocktype ="mid")
        ...
        for block_i, block in enumerate(self.pipe.unet.up_blocks):
            ...
            for (layer_i, attention) in enumerate(getattr(block, "attentions", [])):
                self._wrap_block_forward(attention, "attentions", block_num - block_i - 1, layer_num - layer_i - 1, blocktype ="up")
            for (layer_i, resnet) in enumerate(getattr(block, "resnets", [])):
                self._wrap_block_forward(resnet, "resnet", block_num - block_i - 1, layer_num - layer_i - 1, blocktype ="up")
            for upsampler in getattr(block, "upsamplers", []) if block.upsamplers else []:
                self._wrap_block_forward(upsampler, "upsampler", block_num - block_i - 1, 0, blocktype ="up")
            self._wrap_block_forward(block, "block", block_num - block_i - 1, 0, blocktype ="up")
